...
AppHandler::AppHandler(string path) : Handler(path) {
  mTempDir = "/tmp/" + get_hash();
  std::filesystem::create_directories(mTempDir);
}

std::string AppHandler::extract(string path) {
...
std::shared_ptr<Info> AppHandler::info() {
  if (mInfo != nullptr) {
    return mInfo;
  }
...
void AppHandler::patch_desktop(string src, string dest) {
  string data;
